...
SELECT t.Name Song_Name ,g.Name Song_Genre  -- yine farklı bir isim verdik
FROM tracks t    -- t olarak kısalttık
JOIN genres g   -- g olarak kısalttık
...
FROM invoices i 
JOIN customers c  -- hiç bir şey yazmazsak inner join (default) olarak algılar.
...
FROM artists art
left JOIN albums alb        -- left yaptık çünkü artists tablosundaki bütün verileri görmek için...
...
CREATE TABLE personel  -- "if not exist" eğer yoksa oluştur demek
    (first_name VARCHAR(15),
    last_name TEXT,
    age INT,
    hire_date DATE);

	INSERT INTO personel(first_name,last_name,age,hire_date)
	VALUES	('Mehmet','Çelik',31,2021-12-23),
			('Emrah','Çelik',41,2020-10-10),
			('Enver','Çelik',68,2019-11-20),
			('Fadime','Çelik',66,2011-09-13);
/*------------------------------------------------------------------------------------------
	/*  DROP TABLE
	/*------------------------------------------------------------------------------------------
	/* personel tablosunu siliniz */

DROP TABLE vacation_plan2;   -- "if exist" eğer varsa sil demek 
...
VALUES	(48,'TR','Happy Hotel',1,7,4500),
		(35,'TR','Swiss Hotel',2,14,6000);
...
/*vacation_plan tablosunu place_id sutunu PK ve employee_id sutununu ise FK olarak  değiştirirek
	...
	vacation_length INT,
	budget REAL,
	PRIMARY KEY(id)/*AUTOINCREMENT*/   -- otomatik arttırımlı demek
	...
	INSERT INTO vacation_plan2 VALUES(2,48,'TR','COMFORT HOTEL',8,10,7000);

	INSERT INTO vacation_plan2 VALUES(3,35,'TR','x hotel',1,6,8000);
